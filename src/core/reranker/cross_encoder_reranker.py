...
class CrossEncoderRerankerModel(BaseReranker):
    """Generic implementation for any Cross-Encoder model (e.g., BGE, Jina, MiniLM)."""

    def __init__(self, config: Dict[str, Any]):
        model_path = config.get("model_path")
        if not model_path:
            raise ValueError("model_path is required for CrossEncoder configuration.")
    ...
    def rerank(self, query: str, chunks: List[Chunk], top_k: int = 5) -> List[Chunk]:
        ...
        # Format inputs as pairs for the CrossEncoder
        ...
        # Update Chunk objects with newly computed scores
        for chunk, score in zip(chunks, scores):
            chunk.score = float(score)

        # Sort chunks in descending order based on the relevance score
