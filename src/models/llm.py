...
class QwenLocalLLM(BaseLLM):
    ...
    def parse_query(self, raw_query: str) -> ParsedQuery:
        ...
        try:
            ...
            print(f"Warning: Failed to parse JSON. Raw output: {output_text}")
    ...
    def generate_answer(self, query: str, context_chunks: List[Chunk]) -> str:
        ...
        """
        print(f"Generating answer using {len(context_chunks)} chunks...")
        ...
        response = self.llm(
            prompt, max_tokens=800, temperature=0.3, stop=["<|im_end|>"]
        )
